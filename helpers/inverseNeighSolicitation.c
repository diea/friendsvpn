...
int main(int argc,const char* argv[]) {
    ...
    if (argc<3) {
        fprintf(stderr,"usage: invNbSol <interface> <ipv6-src> <mac-dst>\n");
        exit(1);
    }
    ...
    struct inverseNeighbor_req nreq;
    ...
    while (((token = strsep(&argv[3], ":")) != NULL) && (i < 6)) {
        ...
        nreq.destLL.ether_shost[i] = header.ether_dhost[i];
        i++;
    }
    ...
    pHeader.payload_len = htonl(sizeof(struct inverseNeighbor_req));
    pHeader.nextHeader = ip6.ip6_nxt;

    void* bufferChecksum = malloc(sizeof(struct ipv6upper) + sizeof(struct inverseNeighbor_req));
    memcpy(bufferChecksum, &pHeader, sizeof(struct ipv6upper));
    memcpy(bufferChecksum + sizeof(struct ipv6upper), &nreq, sizeof(struct inverseNeighbor_req));

    nreq.checksum = ~(checksum(bufferChecksum, sizeof(struct ipv6upper) + sizeof(struct inverseNeighbor_req)));

    // Combine the Ethernet header IP and ICMPv6 request into a contiguous block.
    unsigned char frame[sizeof(struct ether_header)+sizeof(struct ipv6hdr)+sizeof(struct inverseNeighbor_req)];
    memcpy(frame, &header ,sizeof(struct ether_header));
    memcpy(frame + sizeof(struct ether_header), &ip6 ,sizeof(struct ipv6hdr));
    memcpy(frame + sizeof(struct ether_header) + sizeof(struct ipv6hdr), &nreq, sizeof(struct inverseNeighbor_req));
