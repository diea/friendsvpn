...
void* DataPlaneConnection::connection_handle() {
    ssize_t len;
...
int DataPlaneConnection::verify_cookie(SSL *ssl, unsigned char *cookie, unsigned int cookie_len) {
...
int DataPlaneConnection::generate_cookie(SSL *ssl, unsigned char *cookie, unsigned int *cookie_len) {
    unsigned char *buffer, result[EVP_MAX_MD_SIZE];
        unsigned int length = 0, resultlength;
        union {
            struct sockaddr_storage ss;
